...
func subWord(word uint32, box *[16][16]byte) (result uint32) {
	result = uint32(box[word>>28][(word>>24)&15]) << 24
	result |= uint32(box[(word>>20)&15][(word>>16)&15]) << 16
	result |= uint32(box[(word>>12)&15][(word>>8)&15]) << 8
	result |= uint32(box[(word>>4)&15][(word)&15])
	return result
}

func subBytes(state []uint32) []uint32 {
	for i := 0; i < len(state); i++ {
		state[i] = subWord(state[i], &SBOX)
	}
	return state
}
...
func invSubBytes(state []uint32) []uint32 {
	for i := 0; i < len(state); i++ {
		state[i] = subWord(state[i], &INV_SBOX)
	}
	return state
}
