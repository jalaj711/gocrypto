...
// TestKeyExpansion128 tests the key expansion function for 10 round AES-128
func TestKeyExpansion128(t *testing.T) {
	...
	for i := 0; i < 44; i++ {
		if expanded[i] != expected[i] {
			t.Fatalf("Testcase: key=%x, result: %x, expected: %x, i=%d \n", key, expanded[i], expected[i], i)
		}
	}
}

// TestKeyExpansion192 tests the key expansion function for 12 round AES-192
...
// TestKeyExpansion256 tests the key expansion function for 14 round AES-256
